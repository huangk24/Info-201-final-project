...
q3_ui <- tabPanel(
  ...
  tags$blockquote(paste0(
    "The more attention a movie is given by critics correlates to how popular ",
    "a movie is with Imdb users. Imdb users are more likely to leave a review ",
    "rank a movie or positively rank movies with a large critical reception. ",
    "In addition to this, movies with a PG rating recieve more reviews from ",
    "critics and are more popular with users as well. Click through to see ",
    "how critical attention is correlated with different types of movies too."
  )),
    ...
    sidebarPanel(
      selectInput("review",
        ...
        choices = c(
          ...
          "Number of Imdb User Reviews" = "num_user_for_reviews",
...
q3_plot <- function(review, color_choice) {
  ...
  d <- ggplot(data = df) +
    ...
    labs(x = "Number of Critic Reviews", y = "Measure of Review",
         color = "Color")
  # return plot
  d
}
